    ...
    foreach ($refs as $index => $folder) {
        ...
        if (count($reference_directories) > 0)
        {
            ...
            foreach ($reference_array as $reference => $value)
            {
                ...
                $references_text .= '<a target="'.$client.'" href="' . $url . '/view-pictures.php?client=' . $client . '&reference=' . $reference . '"><span class="badge">' . $reference . '</span></a>&nbsp;';
            }
            if($_REQUEST['debug'])
            {
...
{
    if($client=='CHEVIGNON' || $client=='SANMARINA')
        $s = explode("_", $image);
    elseif($client=='GALERIES_LAFAYETTE')
    {
        $s = explode("_", $image); $s[0]=$s[1];
    } 
    elseif($client=='LEBONMARCHE' || $client=='ANDRE')
    {
        $s = explode("_", $image);
    }  
