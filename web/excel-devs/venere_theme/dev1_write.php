...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='zip'){
		odownloadZIP($_GET['file'], VENERE_THEME_WRITER_FILE_PATH."/dev1/", "dev1.php") ;
	}else
		odownloadXLS($_GET['file'], VENERE_THEME_WRITER_FILE_PATH."/dev1/".$_GET['folder']."/", "dev1.php") ;
...
if(isset($_POST['submit']))
{
    ...
    if(($ext == 'xlsx') || ($ext == 'xls'))
    {
      ...
      // echo "<pre>";print_r ($fill_colors[1]); echo "</pre>";
        ...
        if(sizeof($final_array)>0)    
        {
    	    ...
    	    $srcPath=VENERE_THEME_WRITER_FILE_PATH."/dev1/".$rand."/";
			$srcFile=VENERE_THEME_WRITER_FILE_PATH."/dev1/".$rand."/".$rand.".xlsx";
//			$srcFileZip=HOTELS_WRITER_FILE_PATH2."/dev1/".$rand.".zip";
    	   ...
    	   foreach($final_array as $key=>$arr){
			  ...
			  if($key == 1){
				 ...
				 //  print_r($header_2);exit;
				  ...
				  $final_array[$key][0] = "http://clients.edit-place.com/excel-devs/venere_theme/refdocs.php?client=VENERE_THEME&folder=".$rand."&file=".$docxfile;
				  $theme=($final_array[$key][17]!='')? $final_array[$key][17]: 'cheap';
				  if($final_array[$key][17]=="B&B"){
					  $final_array[$key][1]="http://clients.edit-place.com/CLIENTS/VENERE/BRIEF_CITY_and_B&B.docx";
				  }
				  
				  $theme=strtolower(str_replace(" ","",trim($theme))); // remove space and convert to lower case to match strings
				  
				  //echo $titles[$lang]['title1'][$theme];
				  
				  /* LANG --- THEME --- BASED TITLES  */
				  //$final_array[$key][12]= "Comment trouver le meilleur ".$final_array[$key][11]." avec Venere ?";
				  //TITLE1
				  $final_array[$key][12]=str_replace("[Main Keyword]",$final_array[$key][11], (isset($titles[$lang]['title1'][$theme]))?$titles[$lang]['title1'][$theme]:$titles[$lang]['title1']['default']);
				  
				  //echo $final_array[$key][12]; exit;
				  
				  //TITLE2
				   if($theme == 'train_station' || $theme == 'airport' || $theme == 'landmark'){
					//echo "HERE"; 
				  	$final_array[$key][13]=str_replace("[GEO_NAME]",$final_array[$key][7], (isset($titles[$lang]['title2'][$theme]))?$titles[$lang]['title2'][$theme]:$titles[$lang]['title2']['default']);
				  	$final_array[$key][13]=str_replace("[CITY]",$final_array[$key][8],$final_array[$key][13]);
				 
				  }else{
					$final_array[$key][13]=str_replace("[CITY]",$final_array[$key][7], (isset($titles[$lang]['title2'][$theme]))?$titles[$lang]['title2'][$theme]:$titles[$lang]['title2']['default']);
				  }
				  //TITLE3
				  
				  if($theme == 'train_station' || $theme == 'airport' || $theme == 'landmark'){
				    $final_array[$key][14]=str_replace("[GEO_NAME]",$final_array[$key][7], (isset($titles[$lang]['title3'][$theme]))?$titles[$lang]['title3'][$theme]:$titles[$lang]['title3']['default']);
				  	$final_array[$key][14]=str_replace("[CITY]",$final_array[$key][8],$final_array[$key][14]);
				  }else{
					$final_array[$key][14]=str_replace("[CITY]",$final_array[$key][7], (isset($titles[$lang]['title3'][$theme]))?$titles[$lang]['title3'][$theme]:$titles[$lang]['title3']['default']);
				  }	
				  
				 // $final_array[$key][14]=(trim($final_array[$key][17])=="B&B")? "Pourquoi rester dans un B&B est-ce pratique lorsque l’on visite ".$final_array[$key][7] :"Comment se loger à ".$final_array[$key][7]." lorsqu'on voyage avec un budget serré ? ";
			  }
			  $l++;
		   }
   ...
   */
   ...
   function columns_included_for_docx($array_data,$hd,$titles,$lang){
	   ...
	   $array_data1=array();
	   $array_data1[1]= $array_data[17];
	   $array_data1[2]= $array_data[3];
	   $array_data1[3]= $array_data[5];
	   $array_data1[4]= $array_data[6];
	   $array_data1[5]= $array_data[15];
	   $array_data1[6]= $array_data[16];
	   
	   $theme=strtolower(str_replace(" ","",trim($array_data[17])));
	   
	   if($hd>2){
			//TITLE1
			$array_data1[7]=str_replace("[Main Keyword]",$array_data[11], (isset($titles[$lang]['title1'][$theme]))?$titles[$lang]['title1'][$theme]:$titles[$lang]['title1']['default']);
				  
	   }else{
		  $array_data1[7]= $array_data[12];
	   }
	   
	   if($hd == 1) { $array_data1[8]=7; } else if($hd == 2) { $array_data1[8]="Paragraph 1"; } else { $array_data1[8]=""; }
		if($hd>2){
		   //TITLE2
			if($theme == 'train_station' || $theme == 'airport' || $theme == 'landmark'){
				$array_data1[9]=str_replace("[GEO_NAME]",$array_data[7], (isset($titles[$lang]['title2'][$theme]))?$titles[$lang]['title2'][$theme]:$titles[$lang]['title2']['default']);
				$array_data1[9]=str_replace("[CITY]",$final_array[$key][8],$array_data1[9]);
				 
			}else{
				$array_data1[9]=str_replace("[CITY]",$array_data[7], (isset($titles[$lang]['title2'][$theme]))?$titles[$lang]['title2'][$theme]:$titles[$lang]['title2']['default']);
			}
		}else{
			$array_data1[9]=$array_data[13];
		}
	   
	   if($hd == 1) { $array_data1[10]=9; } else if($hd == 2) { $array_data1[10]="Paragraph 2"; } else { $array_data1[10]=""; }
	   if($hd>2){  
		  //TITLE3
		if($theme == 'train_station' || $theme == 'airport' || $theme == 'landmark'){
			$array_data1[11]=str_replace("[GEO_NAME]",$array_data[7], (isset($titles[$lang]['title3'][$theme]))?$titles[$lang]['title3'][$theme]:$titles[$lang]['title3']['default']);
			$array_data1[11]=str_replace("[CITY]",$array_data[8], $array_data1[11]);
		}else
		{
			$array_data1[11]=str_replace("[CITY]",$array_data[7], (isset($titles[$lang]['title3'][$theme]))?$titles[$lang]['title3'][$theme]:$titles[$lang]['title3']['default']);
		}
	   }else{
		   $array_data1[11]=$array_data[14];
	   }
	   
	   if($hd == 1) { $array_data1[12]=11; } else if($hd == 2) { $array_data1[12]="Paragraph 3"; } else { $array_data1[12]=""; }
   ...
   */
   ...
   function columns_color_included_for_docx($array_data){
	   ...
	   $array_data1=array();
	   $array_data1[1]= $array_data[18];
	   $array_data1[2]= $array_data[4];
	   $array_data1[3]= $array_data[6];
	   $array_data1[4]= $array_data[7];
	   $array_data1[5]= $array_data[16];
	   $array_data1[6]= $array_data[17];
	   $array_data1[7]= $array_data[13];
	   $array_data1[8]= $array_data[13];
	   $array_data1[9]= $array_data[14];
	   $array_data1[10]= $array_data[14];
	   $array_data1[11]= $array_data[15];
