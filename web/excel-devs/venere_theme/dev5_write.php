...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], VENERE_THEME_WRITER_FILE_PATH."/dev5/", "dev5.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   VENERE_THEME_WRITER_FILE_PATH."/dev5/"."Venere-" . date('ymd')."-".uniqid().".".$ext ;
		  ...
		  if($ext=='rar')
			...
			foreach($docx_files as $lkey=>$docx){
				...
				for($i=0;$i<sizeof($dummy_array);$i++){
					if($j==1){
				       ...
				       $final_array[$j][$i] = ($i == 6 || $i == 7  || $i == 9 || $i == 11 || $i == 13 || $i == 15 || $i == 17 || $i == 19 || $i == 21 || $i == 23) ? modify_content($dummy_array[$i+1][1],$i) : $dummy_array[$i+1][1];  					
            ...
            $rand="Venere-customer-template-delivery-".uniqid();
    	    $srcPath=VENERE_THEME_WRITER_FILE_PATH."/dev5/".$rand."/";
			$srcFile=VENERE_THEME_WRITER_FILE_PATH."/dev5/".$rand.".xlsx";
            ...
            if(file_exists($srcFile)) {
			  header("Location:dev5.php?msg=success&folder=".$rand."&file=".$rand.".xlsx");
			} else {
				  
				  header("Location:dev5.php?msg=error");
			}
        ...
        {
            header("Location:dev5.php?msg=file_error");
        }
    	
}
else
    header("Location:dev5.php");
...
function modify_content($value,$col){
	//$value = str_replace("NA","",$value);
	$value = ($col == 6) ? "<h1>".$value."</h1>" : $value;		
	$value = (($col == 7 || $col == 9 || $col == 11 || $col == 13 || $col == 15 || $col == 17 || $col == 19 || $col == 21 || $col == 23) && trim($value) != "NA") ? "<br><br><b>".$value."</b><br><br>" : str_replace("NA","",$value);;
	return $value;
}	
...
function readDocx($filePath) {
   ...
   */ 
	...
	function writeXlsxVenereDev2($data,$file_path,$anchor_cols=null)
    {
        ...
        $columns_required = array('G','H','I','J','K','L','O','P','Q','R','S','T','U','V','W','X','Y');
        $rowCount=0;
