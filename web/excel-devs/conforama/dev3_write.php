<?php
/**
 * Conforama delivery file xlsx contain all data of docx files data.
 *
 * PHP version 5
 * 
 * @package    ClientDevs
...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], CONFORAMA_WRITER_FILE_PATH."/dev3/", "dev3.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   CONFORAMA_WRITER_FILE_PATH."/dev3/"."Conforama-" . date('ymd')."-".uniqid().".".$ext ;
	...
	//			for($i=0;$i<3;$i++){
					if($j==1){
				       ...
				       $final_array[$j-1][0] = "Colonne vide";
				       $final_array[$j-1][1] = "Code produit (obligatoire)";
				       $final_array[$j-1][2] = "Description";
				       
				       $final_array[$j][0] = "";  					
				       $final_array[$j][1] = $dummy_array[1][1];  					
				       $final_array[$j][2] = modify_content($dummy_array[9][1],9)."<br />".modify_content($dummy_array[10][1],10)."<br />".modify_content($dummy_array[11][1],11)."<br />";  					
            ...
            $rand="conforama-delivery-".uniqid();
    	    $srcPath=CONFORAMA_WRITER_FILE_PATH."/dev3/".$rand."/";
			$srcFile=CONFORAMA_WRITER_FILE_PATH."/dev3/".$rand.".xlsx";
			
			
			writeXlsxConforamaDev3($final_array,$srcFile);
   ...
   * 
   *  Modify convert to htmlentity
   *  @param $value -  value of column cell
   *  @return $value - return the $value with tags <strong> and <b>
   */	
...
function modify_content($value,$col){
	   ...
	   if (getOS($_SERVER['HTTP_USER_AGENT']) == 'Windows')
			$value = htmlentities($value, ENT_COMPAT,'UTF-8', true);
	   		...
	   		$value = htmlentities(utf8_decode($value), ENT_COMPAT,'UTF-8', true);
	   ...
	   if($col == 10 || $col == 11){
			$value = str_replace("<strong>","<br /><strong>",$value);
			$value = str_replace("</strong>","<br /></strong>",$value);
	   }		
	   //echo $value;
	   return $value;
...
function readDocx($filePath) {
   ...
   */ 
	...
	function writeXlsxConforamaDev3($data,$file_path,$anchor_cols=null)
    {
