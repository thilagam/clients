			...
			foreach($docx_files as $lkey=>$docx){
				...
				for($i=0;$i<sizeof($dummy_array);$i++){
					if($j==1){
				      ...
				      $final_array[$j][$i] = ($i+1 >= 10 || $i+1 <= 16 ) ? modify_content($dummy_array[$i+1][1],$i+1) : $dummy_array[$i+1][1];   
				    }else{
...
function modify_content($value,$col){
	...
	$value = ($col == 10 || $col == 12 || $col == 14) ? "<h2>".$value."</h2>" : $value;
	$value = ($col == 11 || $col == 13 || $col == 13) ? "<p>".$value."</p>" : $value;
	return $value;
}	
