...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], PERNODRICARD_WRITER_FILE_PATH."/dev2/", "dev2.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   PERNODRICARD_WRITER_FILE_PATH."/dev2/"."pernodRicard-" . date('ymd')."-".uniqid().".".$ext ;
		  ...
		  if($ext=='rar')
			...
			foreach($docx_files as $lkey=>$docx){
				...
				for($i=2;$i<sizeof($dummy_array)-1;$i=$i+2)
				{
					if(!empty($dummy_array[$i+1][1]) && !empty($dummy_array[$i][0]) && !empty($language))
						$final_array[] = "('".$pernodricard->escapeString($dummy_array[$i][0])."','".$pernodricard->escapeString($language)."','".$pernodricard->escapeString($dummy_array[$i+1][1])."')";  // crete array for insertion
			    }
				$j++;
			}


			$pathFile=$pernodricard->insertTranslations($final_array);
		    //echo "<pre>";print_r($final_array); exit;  
		    
            if(file_exists($srcFile)) {
			  header("Location:dev2.php?msg=success");
			} else {
...
foreach($matches as $key => $value){
  ...
  $value=ltrim($value,'[');
	...
	$arrData='';
  //echo "<pre>"; print_r($matches2);exit;
	//$htmldata.="<td >".$ind."</td>";
