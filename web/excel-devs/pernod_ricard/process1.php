...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
  if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='zip'){
    odownloadZIP($_GET['file'], PERNODRICARD_WRITER_FILE_PATH."/dev1/", "dev1.php") ;
  }
...
{ 
    ...
    if($file1['extension']=='xlsx')
    {
      ...
      $pathFile=$pernodricard->insertKeywords($data[0][0],$columns);
      ...
      $basiclib->zip_creation($pathFile[0],$pathFile[1],'docx');
    	...
    	if (!empty($pathFile))
    	 	header("Location:dev1.php?client=PERNODRICARD&msg=success&file1=".basename($pathFile[1]));
    	else
    	 	header("Location:dev1.php?client=PERNODRICARD&msg=error");
    }
}
else
    header("Location:dev1.php?client=PERNODRICARD");