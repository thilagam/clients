...
?>
<?php
$client_source_path=constant("LBMCHE_REF_SOURCE_PATH");
$client_config_file=constant("LBMCHE_REF_CONFIG_FILE");	
...
if(isset($_POST['upload']))
{
	...
	if(isset($_FILES['excel_file']))
	{
		...
		$tmpName = "LEBONMARCHE_SOURCE_tmp.xlsx";
		if(!is_numeric($ref)&&$ref==0){$err_ref = "Erreur : vous devez entrer un numéro de colonne";$test=false;}
		if(strrchr($filename,'.')!=".xlsx"){$err_file = " Erreur : vous devez charger un fichier XLSX";$test=false;}
		...
		$old_name2 = $client_source_path."/LEBONMARCHE_SOURCE_tmp.xlsx";	
		
		if(file_exists($old_name2))
		{
			//current becomes old
			$old_name1 = "$client_source_path/LEBONMARCHE_SOURCE.xlsx";
			$new_name1 = "$client_source_path/LEBONMARCHE_SOURCE"."_".date("YmdHis").".xlsx";
			rename($old_name1,$new_name1);
			chmod($new_name1,0777);
			
			
			//temp become new
			$new_name2 = "$client_source_path/LEBONMARCHE_SOURCE.xlsx";
		...
		header("Location:upload-lbm-source.php");
	}
	else
		header("Location:upload-lbm-source.php");	
}
else
{
	...
	$pattern = '{'.$client_source_path.'/LEBONMARCHE_SOURCE.*,'.$client_source_path.'/LEBONMARCHE_SOURCE[\_][!A-Za-z]*.xlsx}';
	//$pattern = $client_source_path.'/LEBONMARCHE_SOURCE*.*';
...
<div class="span10 content">				
	<h2 class="heading">LEBONMARCHE  :: R&Eacute;F&Eacute;RENTIEL FILE UPLOAD</h2>  	
	...
	<div class="span11">
		<form class="form-horizontal" name="uploadexcel" action="" method="POST" enctype="multipart/form-data" onsubmit="return checkfile();">
			<div class="control-group <?if($err_file)echo "error" ?>">
				<label class="control-label" for="excel_file">LBM Source XLSX</label>
				<div class="controls">
	...
	<div class="span11">			
		<h5>Details des fichiers ref sources pour LEBONMARCHE</h5>
...
function checkfile()
{
	var error=0;
	var msg='';
	if((document.uploadexcel.excel_file.value.match(/(.xlsx)$/i)))
		{
			error=error;
		}
		else
		{
			msg=msg+"please upload Excel file (only .xlsx file) \n";
			error=error+1;
		}
