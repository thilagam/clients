<?php
/**
 * Venere delivery file xlsx contain all data of docx files data.
 *
 * PHP version 5
 * 
 * @package    ClientDevs
...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], VENERE_NEW_WRITER_FILE_PATH."/dev2/", "dev2.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   VENERE_NEW_WRITER_FILE_PATH."/dev2/"."Venere-" . date('ymd')."-".uniqid().".".$ext ;
		  ...
		  if($ext=='rar')
			...
			foreach($docx_files as $lkey=>$docx){
				...
				$final_array[$j][$i] = ($i+1 == 5 || $i+1 == 7 || $i+1 == 9) ? modify_content($dummy_array[$i+1][1],$i+1) : $dummy_array[$i+1][1];   
				    }else{
    	    ...
    	    $srcPath=VENERE_NEW_WRITER_FILE_PATH."/dev2/".$rand."/";
			$srcFile=VENERE_NEW_WRITER_FILE_PATH."/dev2/".$rand.".xlsx";
   ...
   * 
   *  Modify content for column 6 8 10
...
function modify_content($value,$col){
	$value = ($col == 5) ? "<b>".$value."</b><br><br>" : $value;		
	$value = ($col == 7 || $col == 9) ? "<br><br><b>".$value."</b><br><br>" : $value;
	return $value;
}	
...
function readDocx($filePath) {
   ...
   */ 
	...
	function writeXlsxVenereDev2($data,$file_path,$anchor_cols=null)
    {
        ...
        $columns_required = array('E','F','G','H','I','J');
        $rowCount=0;
