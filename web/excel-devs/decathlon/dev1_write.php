...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='zip'){
		odownloadZIP($_GET['file'], DECATHLON_WRITER_FILE_PATH."/dev1/", "dev1.php") ;
	}else
		odownloadXLS($_GET['file'], DECATHLON_WRITER_FILE_PATH."/dev1/".$_GET['folder']."/", "dev1.php") ;
...
if(isset($_POST['submit']))
{
    ...
    if(($ext == 'xlsx') || ($ext == 'xls'))
    {
        ...
        if(sizeof($final_array)>0)    
        {
    	    ...
    	    $srcPath=DECATHLON_WRITER_FILE_PATH."/dev1/".$rand."/";
			$srcFile=DECATHLON_WRITER_FILE_PATH."/dev1/".$rand."/".$rand.".xlsx";
//			$srcFileZip=HOTELS_WRITER_FILE_PATH2."/dev1/".$rand.".zip";
    	  ...
    	  $l=0;
			...
			//echo "<pre>";print_r($final_array[2]);
    	   ...
    	   foreach($final_array as $key=>$arr){
			  ...
			  if($key == 1){
				 ...
				 //  print_r($header_2);exit;
				  ...
				  $data[1]=$decathlon->nextArticleId();
				 ...
				 // $final_array[$key][0] = "http://clients.edit-place.com/excel-devs/decathlon/refdocs.php?client=decathlon&folder=".$rand."&file=".$docxfile;
				  array_unshift($final_array[$key], "http://clients.edit-place.com/excel-devs/decathlon/refdocs.php?client=decathlon&folder=".$rand."&file=".$docxfile);
				 $decathlon->createArticle($data,$arr,$docxfile);
			  }
			  $l++;
		   }
   ...
   */
   ...
   function columns_included_for_docx($array_data,$hd,$titles,$lang){
	   ...
	   $array_data1[7]= $array_data[8];
	   $array_data1[8]= $array_data[10];
	   $array_data1[9]= '';
	   $array_data1[10]= '';
	   $array_data1[11]= '';
	   $array_data1[12]= '';
	   ...
	   if($hd == 1) { $array_data1[9]=8; } else if($hd == 2) { $array_data1[9]="Sous-Titre 1"; } else { $array_data1[9]=''; }
	   
	   if($hd == 1) { $array_data1[11]=10; } else if($hd == 2) { $array_data1[11]="Sous-Titre 2"; } else { $array_data1[11]=''; }
	    
	   if($hd == 1) { $array_data1[13]=12; } else if($hd == 2) { $array_data1[13]="Sous-Titre 3"; } else { $array_data1[13]=''; }
	   
	   
	   if($hd == 1) { $array_data1[10]=9; } else if($hd == 2) { $array_data1[10]="Paragraphe 1"; } else { $array_data1[10]=''; }
	   
	   if($hd == 1) { $array_data1[12]=11; } else if($hd == 2) { $array_data1[12]="Paragraphe 2"; } else { $array_data1[12]=''; }
	    
	   if($hd == 1) { $array_data1[14]=13; } else if($hd == 2) { $array_data1[14]="Paragraphe 3"; } else { $array_data1[14]=''; }
	   
	  // echo "<pre>"; print_r($array_data1);
	   return $array_data1;
   }
   ...
   */
   ...
   function columns_color_included_for_docx($array_data){
	...
	//	echo "<pre>"; print_r($array_data);
	   // 1  ARTICLE ID	
	   // 2  mot clé attribué	
	   // 3  url d'implémentation	
	   // 4  Page qui se positionne	
	   // 5  Remarque	
	   // 6  nom du doc	
	   // 7  proposition de metatitle	
	   // 8  NBCAR METATITLE	
	   // 9  proposition de meta description	
	   // 11 NBCAR METADESCRIPTION	
	   // 12 proposition de h1	
	   // 13 SOUS-TITRE 1	
	   // 14 PARAGRAPHE 1	
	   // 15 SOUS-TITRE 2	
	   // 16 PARAGRAPHE 2	
	   ...
	   $array_data1=array();
	   $array_data1[1]= $array_data[1]; //article id 
	   $array_data1[2]= $array_data[2]; // Mot clé attribué
	   $array_data1[3]= $array_data[3]; // URL d'implémentation
	   $array_data1[4]= $array_data[4]; // Page qui se positionne
	   $array_data1[5]= $array_data[5]; // Remarque
	   $array_data1[6]= $array_data[7]; // Metatitle
	   $array_data1[7]= $array_data[9]; // Metadescription
	   $array_data1[8]= $array_data[11]; // Titre
	   $array_data1[9]= $array_data[12]; // Sous-titre 1
	   $array_data1[10]= $array_data[13]; // Paragraphe 1 
	   $array_data1[11]= $array_data[14]; // Sous-titre 2
	   $array_data1[12]= $array_data[15]; // Paragraphe 2
	   $array_data1[13]= $array_data[16]; //Sous-titre 3
	   $array_data1[14]= $array_data[17]; // Paragraphe 3
