...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], DECATHLON_WRITER_FILE_PATH."/dev2/", "dev2.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   DECATHLON_WRITER_FILE_PATH."/dev2/"."Bestwestern-" . date('ymd')."-".uniqid().".".$ext ;
      ...
      $docx_files = all_docx_files($unzip_dir);
			...
			$final_array = array( 
                            array(
                                  "ARTICLE ID",
                                  "mot clé attribué",
                                  "url d'implémentation",
                                  "Page qui se positionne",
                                  "Remarque",
                                  "nom du doc",
                                  "proposition de metatitle",
                                  "NBCAR METATITLE",
                                  "proposition de meta description",
                                  "NBCAR METADESCRIPTION",
                                  "proposition de h1",
                                  "SOUS-TITRE 1",
                                  "PARAGRAPHE 1",
                                  "SOUS-TITRE 2",
      ...
      $j=1; 
			...
			$newFinalArray=$final_array;//array(array("ORIGINE","DESTINATION","PARENT","TITRE","INTRO","TITRE_concat","concat","CANONICAL","META_TITRE","META_DESC","META_INDEX","DATA_SUMO","CATEGORIE","LANGUE","PAYS","POIDS"));
      ...
      /*
        ...
        * As per the Trello card:https://trello.com/c/yppv7Se4/143-decathlon-dev-2-for-delivery,
        https://trello.com/c/hIFLftoe/155-decathlon--update-on-delivery-dev
      ...
      */
      ...
      foreach($docx_files as $lkey=>$docx)
      {
				...
				for($i=1;$i<=sizeof($dummy_array);$i++)
        {
        	if($i==1)
          {
            $sql = "Select * from cl_decathlon_articles where decathlon_article_id=".strip_tags($dummy_array[$i][2]);
            //echo $sql;exit;
        ...
        $concat = "<h2>".str_replace("'","''",toLink(strip_tags($final_array[$j][9])))."</h2>".str_replace("'","''",toLink(paragraph($final_array[$j][10])))."<h2>".str_replace("'","''",toLink(strip_tags($final_array[$j][11])))."</h2>".str_replace("'","''",toLink(paragraph($final_array[$j][12])))."<h2>".str_replace("'","''",toLink(strip_tags($final_array[$j][13])))."</h2>".str_replace("'","''",toLink(paragraph($final_array[$j][14])));
		//echo $concat;exit;
        //echo html_entity_decode($concat);exit;
        $sql1 = "UPDATE cl_decathlon_articles 
                    SET decathlon_meta_title = '<title>".str_replace("'","''",strip_tags($final_array[$j][6]))."</title>',
                        decathlon_meta_desc = '".str_replace("'","''",toLink(strip_tags($final_array[$j][7])))."',
                        decathlon_title = '".str_replace("'","''",strip_tags($final_array[$j][8]))."',
                        decathlon_subtitle_1 = '".str_replace("'","''",strip_tags($final_array[$j][9]))."',
                        decathlon_para_1 = '".str_replace("'","''",toLink(strip_tags($final_array[$j][10])))."',
                        decathlon_subtitle_2 = '".str_replace("'","''",strip_tags($final_array[$j][11]))."',
                        decathlon_para_2 = '".str_replace("'","''",toLink(strip_tags($final_array[$j][12])))."',
                        decathlon_subtitle_3 = '".str_replace("'","''",strip_tags($final_array[$j][13]))."',
                        decathlon_para_3 = '".str_replace("'","''",toLink(strip_tags($final_array[$j][14])))."',
                        decathlon_concat = '".$concat."' 
                  WHERE decathlon_article_id=".strip_tags($final_array[$j][1]);
          //echo $sql1;exit;
         $dbfunctions->mysql_qry($sql1,1); 
          $finalData = array();
          $finalData[] = (!empty($other_data))?$other_data['decathlon_article_id']:strip_tags($final_array[$j][1]);
          $finalData[] = (!empty($other_data))?$other_data['decathlon_mot_attrib']:strip_tags($final_array[$j][2]);
          $finalData[] = (!empty($other_data))?$other_data['decathlon_url']:strip_tags($final_array[$j][3]);
          $finalData[] = (!empty($other_data))?$other_data['decathlon_page_position']:strip_tags($final_array[$j][4]);
          $finalData[] = (!empty($other_data))?$other_data['decathlon_remark']:strip_tags($final_array[$j][5]);
          $finalData[] = '' ; // nom de doc
          //metas 
          $finalData[] = "<title>".checkCharacterCount(strip_tags($final_array[$j][7]),array(0,70))."</title>";
          $finalData[] = '#FORMULA';
          $finalData[] = toLink(checkCharacterCount(strip_tags($final_array[$j][7]),array(160,190)));
          $finalData[] = '#FORMULA';
          $finalData[] = strip_tags($final_array[$j][7]);

          //paras
          $finalData[] =toLink(strip_tags($final_array[$j][9]));
          $finalData[] =toLink(paragraph($final_array[$j][10]));
          $finalData[] =toLink(strip_tags($final_array[$j][11]));
          $finalData[] =toLink(paragraph($final_array[$j][12]));
          $finalData[] =toLink(strip_tags($final_array[$j][13]));
          $finalData[] =toLink(paragraph($final_array[$j][14]));
          $finalData[] =$concat;
          $finalData[] ='';
          $finalData[] ='';
          $finalData[] ='';
      ...
      $rand="decathlon-delivery-".uniqid()."-".date('d-m-y-h-m');
    	$srcPath=DECATHLON_WRITER_FILE_PATH."/dev2/".$rand."/";
			$srcFile=DECATHLON_WRITER_FILE_PATH."/dev2/".$rand.".xlsx";
...
foreach($matches as $key => $value){
  ...
  //$arrData = str_replace("|||||","]",$arrData);
	$arrData = str_replace("&rsquo;","'",$arrData);
...
function readDocx($filePath) {
        ...
        foreach ($data as $k=>$row)
        {
            ...
            {	
              ...
              if(strpos($value, 'ERR-') !== false){  
                $stylArr1 = array('fill' =>array('type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => array('rgb' => 'FF0000')));
                $objPHPExcel->getActiveSheet()->getStyle($col.($rowCount + 1)) -> applyFromArray($stylArr1);
                $objPHPExcel->getActiveSheet()->getStyle(strtoupper(chr(ord($col)+1)).($rowCount+1)) -> applyFromArray($stylArr1);
              ...
              if($value=="#FORMULA"){
                $value="=LEN(".strtoupper(chr(ord($col)-1)).($rowCount+1).")";
  ...
  /**
     ...
     * @return
     */    
    function checkCharacterCount($str,$limit)
    { 
      if (getOS($_SERVER['HTTP_USER_AGENT']) != 'Windows')
      { 
          $str = str_replace("&rsquo;", "'", $str) ;
      }
      $count=mb_strlen(trim($str))."<br>";
