...
?>
<?php
$client_source_path=constant("CAROLL_EP_SOURCE_PATH");
$client_config_file=constant("CAROLL_EP_CONFIG_FILE");
...
{	
	...
	if(isset($_FILES['excel_file']))
	{
		...
		$old_name2 = $client_source_path."/CAROLL_tmp.xls";	
		
		if(file_exists($old_name2))
		{
			//current becomes old
			$old_name1 = "$client_source_path/CAROLL.xls";
			$new_name1 = "$client_source_path/CAROLL"."_".date("YmdHis").".xls";
			rename($old_name1,$new_name1);
			chmod($new_name1,0777);
			
			
			//temp become new
			$new_name2 = "$client_source_path/CAROLL.xls";
...
else
{
	...
	$pattern = '{'.$client_source_path.'/CAROLL.*,'.$client_source_path.'/CAROLL[\_][!A-Za-z]*.xls}';
	//$pattern = $client_source_path.'/CAROLL*.*';
...
<div class="span10 content">				
	<h2 class="heading">CAROLL  :: R&Eacute;F&Eacute;RENTIEL FILE UPLOAD</h2>  	
	...
	<div class="span11">
		<form class="form-horizontal" name="uploadexcel" action="" method="POST" enctype="multipart/form-data" onsubmit="return checkfile();">
			<div class="control-group <?if($err_file)echo "error" ?>">
				<label class="control-label" for="excel_file">Fichier r&eacute;f&eacute;rentiel Caroll</label>
				<div class="controls">
	...
	<?php $header_rows=1; 
      $latest_source_file = display_input_xls_xlsx_file("ep-source-files/CAROLL.xls",6);
...
?>	<?php //echo "<pre>";print_r($latest_source_file); ?>
  <?php if(count($latest_source_file) > 0) { ?>
	...
	<div class="span11">			
		<h5>Details des fichiers sources pour CAROLL</h5>
