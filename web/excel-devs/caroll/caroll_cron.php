...
if ($ep_source_updated == 'yes' || $caroll_source_updated == 'yes' || !empty($folder_id)) {
    ...
    //echo "<pre>";print_r($array_allImages);exit;
    ...
    if ($fcount > 0) {
        foreach ($folders as $dir) {
            ...
            if (is_dir($dir) AND !strpos($dir, '.zip') AND basename($dir) != "caroll-xml-log" AND basename($dir) != "caroll-xml")
            {
                ...
                $exl = file_exists(CAROLL_WRITER_FILE_PATH . $filename . ".xls") ? ".xls" : ".xlsx";
                $exl3 = file_exists(CAROLL_WRITER_FILE_PATH . "/Writer_Final_3_" . $folder . ".xls") ? ".xls" : ".xlsx";
                
                $filename = "Writer_Final_" . $folder;
                $path_file = CAROLL_WRITER_FILE_PATH . "/" . $filename . $exl;
                $path_new_file = CAROLL_WRITER_FILE_PATH . "/" . $filename . ".xlsx";
                $new_name = CAROLL_WRITER_FILE_PATH . "/" . $filename . "_" . date("YmdHis") . $exl;
                $xls3_path = CAROLL_WRITER_FILE_PATH . "/Writer_Final_3_" . $folder . $exl3;
                $xls3_new_path = CAROLL_WRITER_FILE_PATH . "/Writer_Final_3_" . $folder . ".xlsx";
		...
		{
		    echo "<br><br>Creating the file .....$filename.xlsx\n<br>";
		}
		...
		//echo "<pre>";print_r($global_parents_data_file);exit;

                /**** START WRITE XLS INCLUDE ****/      
                
		//echo "<pre>";print_r($img_stack);exit;
                // Getting matches                
...
//echo "<pre>";print_r($xls_array);exit($folder) ;
                ...
                if (file_exists($new_name)) {
                    ...
                    if(count($final_xls)>1)
                    {
						...
						{
							$new_name1 = CAROLL_WRITER_FILE_PATH . "/Writer_Final_3_" . $folder . "_" . date("YmdHis") . $exl3;
    ...
    $wrfile = CAROLL_WRITER_FILE_PATH."/".$wrfilename;
...
function getCarollPrevReferences($folder_id) {
    ...
    {
        $file_path = CAROLL_WRITER_FILE_PATH . "/Writer_Final_" . $id . ".xlsx";
    ...
    {
        $file_path = CAROLL_WRITER_FILE_PATH . "/Writer_Final_3_" . $id . ".xlsx";
...
function getAllCarollMissingRefs($folder_id) {
    ...
    $currentFolderFile = "Writer_Final_" . $folder_id . (file_exists(CAROLL_WRITER_FILE_PATH . "/Writer_Final_" . $folder_id . ".xlsx") ? ".xlsx" : ".xls");
