...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], LA_REDOUTE_WRITER_FILE_PATH."/dev2/", "dev2.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   LA_REDOUTE_WRITER_FILE_PATH."/dev2/"."LaRedout-" . date('ymd')."-".uniqid().".".$ext ;
      ...
      $final_array = array(array(" Article ID","Keywords","URL","MetaTitle","MetaDesc","Title","Seotext","Concatenation"));

      $j=1; 
			$check_empty = "";
						
			$newFinalArray=$final_array;//array(array("ORIGINE","DESTINATION","PARENT","TITRE","INTRO","TITRE_EDITO","EDITO","CANONICAL","META_TITRE","META_DESC","META_INDEX","DATA_SUMO","CATEGORIE","LANGUE","PAYS","POIDS"));
      ...
      foreach($docx_files as $lkey=>$docx)
      {
				...
				for($i=1;$i<=sizeof($dummy_array);$i++)
        {
        	if($i==1)
          {                                                    
            $sql = "Select * from  cl_laredoute_articles where laredoute_article_id=".strip_tags($dummy_array[$i][2]);
            //echo $sql;exit;
      ...
      // echo "<pre>"; print_r($final_array[$j]);
        ...
        $sql1 = "UPDATE  cl_laredoute_articles  
                    SET laredoute_concat = '".$edito."',
                        laredoute_meta_title = '".str_replace("'","''",toLink(strip_tags($final_array[$j][4])))."',
                        laredoute_meta_description = '".str_replace("'","''",toLink(strip_tags($final_array[$j][5])))."',
                        laredoute_title = '".str_replace("'","''",toLink(strip_tags($final_array[$j][6])))."',
                        laredoute_seo_text = '".str_replace("'","''",toLink(strip_tags($final_array[$j][7])))."'
                  WHERE laredoute_article_id=".strip_tags($final_array[$j][1]);
        //echo $sql1;exit;
        ...
        $dbfunctions->mysql_qry($sql1,1); 
          ...
          $finalData[] = toLink(strip_tags($final_array[$j][6]));
      ...
      $rand="laredout-delivery-".uniqid()."-".date('d-m-y-h-m');
    	$srcPath=LA_REDOUTE_WRITER_FILE_PATH."/dev2/".$rand."/";
			$srcFile=LA_REDOUTE_WRITER_FILE_PATH."/dev2/".$rand.".xlsx";
