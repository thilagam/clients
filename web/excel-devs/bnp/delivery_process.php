...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='xlsx'){
		odownloadXLS($_GET['file'], BNP_WRITER_FILE_PATH."/dev2/", "delivery.php") ;
	}
...
if(isset($_POST['submit']))
{
    ...
    if($ext == 'zip' || $ext == 'rar')
    {
		  ...
		  $ext	=	$file1['extension'] ;
		  $srcFile    =   BNP_WRITER_FILE_PATH."/dev2/"."BNP-" . date('ymd')."-".uniqid().".".$ext ;
      ...
      $docx_files = all_docx_files($unzip_dir);
			...
			$final_array = array(array("Article ID ","Localité"," Codes Postaux","Type de Bien (Bureaux/Entrepot/commerce)","Type de transaction (Location/Vente)","Texte"));


      $j=1; 
			$check_empty = "";
						
			$newFinalArray=array(array("Article ID ","Localité"," Codes Postaux","Type de Bien (Bureaux/Entrepot/commerce)","Type de transaction (Location/Vente)","Texte"));
