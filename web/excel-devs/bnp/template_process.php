<?php
/**
 * BNP Writer file Creates Multiple doc files from single xlsx file
 *
 * PHP version 5
 * 
 * @package    ClientDevs
...
if(isset($_GET['action']) && $_GET['action']=='download' && isset($_GET['file']))
	if(pathinfo($_GET['file'], PATHINFO_EXTENSION)=='zip'){
		odownloadZIP($_GET['file'], BNP_WRITER_FILE_PATH."/dev1/", "templates.php") ;
	}else
		odownloadXLS($_GET['file'], BNP_WRITER_FILE_PATH."/dev1/".$_GET['folder']."/", "templates.php") ;
...
if(isset($_POST['submit']))
{
    ...
    if(($ext == 'xlsx') || ($ext == 'xls'))
    {
        ...
        if(sizeof($final_array)>0)    
        {
	      	...
	      	$rand="bnp-".$lang."-".uniqid()."-".date('d-m-y-h-m');
		    $srcPath=BNP_WRITER_FILE_PATH."/dev1/".$rand."/";
			$srcFile=BNP_WRITER_FILE_PATH."/dev1/".$rand."/".$rand.".xlsx";
	//			$srcFileZip=HOTELS_WRITER_FILE_PATH2."/dev1/".$rand.".zip";
			...
			//echo "<pre>";print_r($final_array);exit;
    	   	...
    	   	foreach($final_array as $key=>$arr){
				//echo "<pre>";  
				if($key == 1){
				   $header = $bnp->docxColumns($arr,1,$titles,$lang);
				  // print_r($header);
				   array_unshift($final_array[$key], "0");
				  
				}else if($key == 2){
				   $header_2 = $bnp->docxColumns($arr,2,$titles,$lang);
				   array_unshift($final_array[$key], "URL");
				   //print_r($header_2);exit;
				}else{
				  	$data = $bnp->docxColumns($arr,3,$titles,$lang);
				  	$data[1]=$bnp->nextArticleId();
				 	...
				 	// $final_array[$key][0] = "http://clients.edit-place.com/excel-devs/bnp/refdocs.php?client=bnp&folder=".$rand."&file=".$docxfile;
				  	array_unshift($final_array[$key], "http://clients.edit-place.com/excel-devs/bnp/refdocs.php?client=bnp&folder=".$rand."&file=".$docxfile);
				  	$bnp->createArticle($data,$arr,$docxfile);
				}
				$l++;
		   }
		...
		//print_r ($fill_colors); echo "</pre>"; exit;
		  ...
		  // writeMultiSheets(array($final_array), $srcFile,array('sheet1') ,$sheetnames,$colors); 
            if(file_exists($srcFile)) {
			  header("Location:templates.php?msg=success&folder=".$rand."&file=".$rand.".xlsx");
			} else {
        ...
        {
            header("Location:templates.php?msg=file_error");
        }
    }	
}
else
