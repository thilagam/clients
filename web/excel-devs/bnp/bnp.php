...
include_once(BNP_PATH."/dbfunctions.php");
...
{	
	...
	public function createArticle($data,$allData,$file)
	{
		...
		$insertData=array(
							'bnp_regions'=>$this->escape_quote($data[1]),
							'bnp_territoiry'=>$this->escape_quote($data[2]),
							'bnp_priorite'=>$this->escape_quote($data[3]),
							'bnp_localite'=>$this->escape_quote($data[4]),
							'bnp_codes'=>$this->escape_quote($data[5]),
							'bnp_type'=>$this->escape_quote($data[6]),
							'bnp_transaction'=>$this->escape_quote($data[7]),
							'bnp_master'=>$this->escape_quote($data[8]),
							'bnp_create_date'=>date('Y-m-d H:i:s')
						);
	...
	public function docxColumns($array_data,$hd,$titles,$lang)
	{
	   ...
	   $array_data1[5]= $array_data[6];
	   $array_data1[6]= $array_data[4];
	   $array_data1[7]= $array_data[5];
	   $array_data1[8]= $array_data[8];
	   $array_data1[9]= $array_data[9];
	   $array_data1[10]= '';
	   $array_data1[11]= '';
	   $array_data1[12]= '';
	   $array_data1[13]= '';
